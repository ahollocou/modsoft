...
class PythonModSoft(object):
    ...
    def modularity(self):
        ...
        for node in range(self.n_nodes):
            for neighbor, weight in self.graph_edges[node].iteritems():
                if neighbor == node:
                    weight *= 2.
                for com, p_com in self.p[node].iteritems():
    ...
    def one_step(self):
        ...
        for node in range(self.n_nodes):
            ...
            for neighbor, weight in self.graph_edges[node].iteritems():
                for com, p_com in self.p[neighbor].iteritems():
                    if com not in new_p:
                        new_p[com] = 0
                    new_p[com] += self.resolution * self.learning_rate * weight * p_com
            for com, new_p_com in new_p.iteritems():
